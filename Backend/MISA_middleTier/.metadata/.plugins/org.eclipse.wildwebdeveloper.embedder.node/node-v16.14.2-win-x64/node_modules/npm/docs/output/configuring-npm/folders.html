	...
	<section id="content">
			...
			<div id="_table_of_contents">
				<ul>
					...
					<ul>
						...
						<li><a href="#cycles-conflicts-and-folder-parsimony">Cycles,
								Conflicts, and Folder Parsimony</a></li>
		...
		<div id="_content">
			...
			<ul>
				...
				<li>Install it <strong>globally</strong> if you're going to run
			...
			<p>
				...
				<code>{prefix}</code>
				on Windows. Ensure that path is in your terminal's
				<code>PATH</code>
			...
			<p>
				See <a href="../commands/npm-cache.html"><code>npm cache</code></a>.
			...
			<p>
				Starting at the $PWD, npm will walk up the folder tree checking for
				a folder that contains either a
				<code>package.json</code>
				file, or a
				<code>node_modules</code>
				folder. If such a thing is found, then that is treated as the
				effective "current directory" for the purpose of running npm
				commands. (This behavior is inspired by and similar to git's
			...
			<h4 id="cycles-conflicts-and-folder-parsimony">Cycles,
				Conflicts, and Folder Parsimony</h4>
			<p>
				Cycles are handled using the property of node's module system that
				it walks up the directories looking for
				<code>node_modules</code>
			...
			<p>
				...
				, since when it calls require("bar"), it will get the copy that is
				installed in
			...
			<p>
				...
				if the two "a" packages are different versions. However, without
				repeating the exact same package multiple times, an infinite regress
				will always be prevented.
			</p>
			<p>Another optimization can be made by installing dependencies at
				the highest level possible, below the localized "target" folder.</p>
    ...
    `-- baz (1.2.3) &lt;---[D]
        `-- node_modules
			...
			<p>
				Underneath
				<code>foo -&gt; baz</code>
				[D], quux's [E] folder tree is empty, because its dependency on bar
			...
			<p>
				...
				those items that cannot be found elsewhere. See <a
		...
		<footer id="edit">
			<a
				href="https://github.com/npm/cli/edit/latest/docs/content/configuring-npm/folders.md">
